...
`confirmAppointment`, `markDepositReceived`, `rejectAppointment`, `completeAppointment`, `markNoShow`, `createBlockedSlot`, `deleteBlockedSlot`, `createAvailabilityWindow`, `deleteAvailabilityWindow`, `searchClients`, `createManualAppointment`, `rescheduleAppointment`, `createClientRecord`, `updateClient`, `updateClientNotes`, `createService`, `updateService`, `setServiceActive`, `createExpense`, `updateExpense`, `deleteExpense`, `getAvailableRewards`, `signOut`. Ver `docs/PRD-agendamiento.md` y `docs/HANDOFF-fase-*.md` para el diseño de cada una.
